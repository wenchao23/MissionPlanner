            ...
            if (CMB_status != 0)
            {
                ...
                for (int i = 1; i < Numberofmav; i = i + 1) {
                    for (int j = 1; j < Numberofmav; j = j + 1) {
                        ...
                        if (dis_matrix[mav_tag[i], mav_tag[j]] < 20) {
            ...
            foreach (var port in MainV2.Comports.ToArray())
            {
                ...
                foreach (var mav in port.MAVlist)
                {
                    ...
                    if (CMB_status!= 0)
                    {
                        ...
                        if (d < desried_dis && this.sysid_enter_start[mav.sysid] == 0)
                        {
                            continue;
                        }
                        ...
                        ////collision avoidance
                        //for (int i = index_mav_tag; i > 0; i = i - 1) {
                        //    if (collision_detect[mav_tag[index_mav_tag], mav_tag[i]] == 1){
                        //        //target.lng = mav.cs.lng;
                        //       //target.lat = mav.cs.lat;
                        //        target.Alt = pos[i,2]-5;
